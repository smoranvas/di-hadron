...
def mixed_quantities(E, e_px, e_py, e_pz, h_px, h_py, h_pz,h_pid, h2_px, h2_py, h2_pz,h2_pid, useOriginalPT=False):
...
#new version
def mix_from_singles(df, binvars=''.split(), nbins=1,maxEvents=None, nAssocPerTrigger=1,j0=0,electronCuts=False,maxDiffAngle=2,maxDiffX=.05,maxDiffQ2=999, useOriginalKinematics=False, useOriginalPT=False):
    ...
    for i in range(N):
        ...
        for ii in range(nAssocPerTrigger):
            ...
            # unless useOriginalKinematics
            if not useOriginalKinematics:
                ...
                di = mixed_quantities(df.E[j], df.e_px[j], df.e_py[j], df.e_pz[j],df.h_px[i], df.h_py[i], df.h_pz[i],df.h_pid[i],
                                                df.h_px[j], df.h_py[j], df.h_pz[j],df.h_pid[j])
                d["h2_z"].append(df.h_z[j])
...
if __name__ == '__main__':
    ...
    for arg in sys.argv[2:]:
        if '-N=' in arg:
            ...
            useOriginalKinematics=True
        elif '--originalPT' == arg:
            useOriginalPT=True
    def process(j0,i):
        df_mixed = mix_from_singles(df, binvars=''.split(), nbins=1,maxEvents=maxEvents, nAssocPerTrigger=1,j0=j0,electronCuts=electronCuts,maxDiffX=maxDiffX,maxDiffAngle=maxDiffAngle,maxDiffQ2=maxDiffQ2, useOriginalKinematics=useOriginalKinematics)
    ...
    if not singleThread:
        ...
        df_mixed = mix_from_singles(df, binvars=''.split(), nbins=1,maxEvents=maxEvents, nAssocPerTrigger=nAssocPerTrigger,electronCuts=electronCuts,maxDiffX=maxDiffX,maxDiffAngle=maxDiffAngle,maxDiffQ2=maxDiffQ2, useOriginalKinematics=useOriginalKinematics, useOriginalPT=useOriginalPT)
