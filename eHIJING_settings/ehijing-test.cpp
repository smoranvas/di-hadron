...
public:
  ...
  hadronizer(std::string configFile):pythia(),rd(),gen(rd()),dist(0.,1.){
...
void Output(Pythia & pythia, std::vector<Particle> & plist, std::ofstream & F){
    ...
    for (auto & p : plist) {
	    ...
	    F << " " << p.id() << " "
		 // photon-z frame 
		 << z << " " << kT << " " << prot.phi() << " " << prot.rap() 
		 // Fixed target frame
	      << " " << p.pAbs() << " " << thetalab << " " << philab << std::endl; 
	 }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // config file for hadronizer instance of pythia
    auto configfile2=std::string(argv[9]);
    ...
    pythia.init();
    pythia.readFile(configfile);  //override anything otherwise overwritten
