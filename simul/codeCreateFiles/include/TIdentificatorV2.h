...
// Added by smoran
// The functions FillVlassovCoordinates , get_cc_special_coord and get_vcrpl are used to obtaine the
// 'CC mirror Matching Cut' (used as an altenative way of reducing pi- contamination in the electron sample.)

Bool_t CCMirrorMatching(Int_t k){
	Bool_t result;
	Float_t trig_cctheta, trig_ccphi, thetaP;
	thetaP = FillVlassovCoordinates(k,trig_cctheta, trig_ccphi );
	Int_t MirrorCodeTemp = CCStatus(k);  
	Int_t MirrorCode     = MirrorCodeTemp%1000; 
	result = TMath::Abs(thetaP-(7.306737e+00+1.383339e-01*MirrorCode+3.672767e-04*MirrorCode*MirrorCode) )<2.5;	   
    return result;
}

Float_t FillVlassovCoordinates(Int_t k, Float_t &trig_cctheta,Float_t  &trig_ccphi ){

  Float_t point[3], dir[3], cc_coor[2];
  point[0] = DC_X_SC(k);
  point[1] = DC_Y_SC(k);
  point[2] = DC_Z_SC(k);
    dir[0] = DC_CX_SC(k);
    ...
    dir[2] = DC_CZ_SC(k);
    Float_t theta;
  theta = get_cc_special_coord(point,dir,cc_coor); 
...
Float_t get_cc_special_coord(Float_t *point, Float_t *dir, Float_t *cc_coor){

  Int_t nt;
  Float_t x[3], dist, r, s, ccTheta, ccPhi;
  Float_t cc_pln[3] = { - 0.0007840784063, 0., - 0.001681461571 }; //was static in original code
...
Int_t get_vcrpl(Float_t *r0, Float_t *dir, Float_t *plane_par, Float_t *dist, Float_t *cross_point){
                                                                                                                     
  Double_t a,b,t,c,d[3];
  const Double_t un = 1.0000000000;
  const Float_t vsmall = 0.000001;
  Int_t i, ires;
  a = b = c = 0.;
  *dist = 0.;
  for(i=0;i<3;i++)
  ...
  Bool_t SampFracCheck(Int_t k, TString dataKind, TString targetOption, Double_t sigma) {  
    // Sampling Fraction Cut exclusive for electrons
    // Just in case, Taisiya Mineeva's cuts for sim
    ...
    // final step
    if (TMath::Abs(E/p - MeanE) < sigma*SigmaE) {
      return true;
    } // closure
    return false;
  }
  ...
  Bool_t FidCheckCut(Int_t k) {
    ...
    if (ThetaLab(k, 0) > FidThetaMin(k) && 
         PhiLab(k, 0) > FidPhiMin(k) && 
	 PhiLab(k, 0) < FidPhiMax(k)) {
      return 1;
    } // closure
    return 0;
  }
  
// Parameters for fiducial cuts for pi+. 
  ...
  Double_t FidPhiMinPiPlus(Int_t k)
{
    ...
    if (ThetaLab(k,0) <= FidThetaMinPiPlus(k)) {
        fid_phi_min_val = 60. * sector;
    } else {
        fid_phi_min_val = 60. * sector - FidFuncPiPlus(0,0,k) *
                (1 - 1 / (1 + (ThetaLab(k,0) - FidThetaMinPiPlus(k)) / FidFuncPiPlus(0,1,k)));
    }
...
Double_t FidPhiMaxPiPlus(Int_t k)
{
    ...
    if (ThetaLab(k,0) <= FidThetaMinPiPlus(k)) {
        ...
        fid_phi_max_val = 60. * sector + FidFuncPiPlus(1,0,k) *
                (1 - 1 / (1 + (ThetaLab(k,0) - FidThetaMinPiPlus(k)) / FidFuncPiPlus(1,1,k)));
    }
...
{
    if ( ThetaLab(k, 0) > FidThetaMinPiPlus(k) &&
         PhiLab(k, 0)   > FidPhiMinPiPlus(k)   &&
         PhiLab(k, 0)   < FidPhiMaxPiPlus(k)  )
  ...
  Bool_t ElectronPositronBasicCuts(Int_t k, TString dataKind, TString targetOption, TVector3* ECuvw){
	...
	SampFracCheck(k, dataKind, targetOption,3.0) &&
  ...
  if (ElectronPositronBasicCuts(k, dataKind, targetOption, ECuvw) &&  Charge(k) == -1  
  && W()>2.0 && Q2()>1.0 && Yb()<0.85 && TargTypeHH()!=0 && TMath::Abs(GetCorrectedVert()->Y())<1.4 ){return true;}
  else {return false;}
  }
  Bool_t IsPositron(Int_t k, TString dataKind, TString targetOption, TVector3* ECuvw) {
  if ( ElectronPositronBasicCuts(k, dataKind, targetOption, ECuvw) &&  Charge(k) == 1){  return true;  }
  else {return false;}
  }
  ...
  Bool_t PiPlusPhaseSpace_CC(Int_t k, Double_t P_threshold=2.7 , Int_t NPhe_threshold=15) {
    Double_t P  = Momentum(k);
    Double_t T4 = TimeCorr4(k, 0.13957);
    if (NRowsCC() != 0 && StatCC(k) > 0 &&  Nphe(k) > NPhe_threshold && 
        Chi2CC(k) < 5/57.3 && P >= P_threshold && 
	T4 > -0.6 && T4<(0.45*(P>=P_threshold && P<3.3) + 0.5*(P>3.3 && P<6.0)) ) {
      return true;
    }
    return false;
  }
  ...
  Bool_t IsPiPlus(Int_t k, TVector3* ECuvw) {
	...
	*/
	&&
	TMath::Abs(Z(k)-GetCorrectedVert()->Z() )<3.5 && 
  ...
  Bool_t PiMinusPhaseSpace_SC(Int_t k, TString dataKind) {
    ...
    const Double_t lines_PiMinus[5][2]= {{-0.75, 0.80},  
					 ...
					 {-0.50, 0.44},
					 {-0.50, 0.45}};
   const Double_t lines_PiMinus_sim[5][2]= {{-0.75  , 0.80 } ,
                                             { -0.55  , 0.60 } ,
                                             { -0.55  , 0.65 } ,
                                             { -0.50  , 0.54 } ,
                                             { -0.50  , 0.45 } };  
if (dataKind=="data"){		
       ...
       )
   {
     ...
     if (StatSC(k) > 0 && ((0 < P && P <= 0.5 && T4 >= lines_PiMinus_sim[0][0] && T4 <= lines_PiMinus_sim[0][1]) ||
 ...
 return false;
  ...
  Bool_t PiMinusPhaseSpace_CC(Int_t k) {
    // for high energy pi-, NOT USED, ONLY TOF INFO FOR PI-
    Double_t P  = Momentum(k);
    Double_t T4 = TimeCorr4(k, 0.13957);
  ...
  Bool_t IsPiMinus(Int_t k, TVector3 *ECuvw, TString dataKind) {
	...
	NRowsDC() != 0 && DCStatus(k) > 0 && StatDC(k) > 0 &&
	Momentum(k) < 5.0  &&
	PiMinusPhaseSpace_SC(k, dataKind) && TMath::Abs(Z(k)-GetCorrectedVert()->Z() )<3.5   ) { 
      return true;
    } // closure
    return false;
  }
  ...
  Bool_t IsProton(Int_t k) {
	...
	deltaBetta > (-0.00218 - 3*0.01002)  &&  FidCheckCutPiPlus(k)    ) {
      return true;
    } // closure
    return false;
  }
